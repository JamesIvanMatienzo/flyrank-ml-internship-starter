...
- **Unit of analysis:** One content page per client, aggregated to monthly search performance over June 2026, presumably the latest complete month in the warehouse. Each row represents a unique (client_hash_id, content_hash_id) pair with summed impressions, clicks, weighted average position, and session-weighted engagement rate for that month.
...
- **False positive** (flagging a well-performing page): approximately 1–2 hours of editor time wasted per page reviewed. The page did not need a rewrite, but no permanent damage is done.
...
| Excluded Field | Explanation Why it is Excluded |
|---|---|
...
| Excluded Category | Explanation Why it is Excluded |
|---|---|
...
- underperforming = 1 if ctr_gap > 0, else 0
- ctr_gap = tier_median_ctr - observed_ctr
- log_impressions = log1p(total_impressions)
...
**Target definition:** A page is labeled is_opportunity = 1 if its observed CTR falls below the median CTR for pages in the same position tier AND it receives at least 1,000 monthly impressions.
...
- **False positives (model flags a page that isn't an opportunity):** Concentrated in pos_4_10 pages that have high impressions but average CTR — the model's strongest signal (log_impressions) overrides the weaker position-adjusted CTR signal for these cases.
...
| has_ga4_data | -0.173 | Pages with GA4 data are slightly less likely (better-tracked clients tend to optimize more) |
...
### **How a FlyRank Editor Uses The Results of this Capstone**
